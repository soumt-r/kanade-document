import { readdirSync, readFileSync, statSync, writeFileSync, unlinkSync } from 'fs';
import { join } from 'path';
import { execSync } from 'child_process';
import { Lexer } from './src/utils/kanade/lexer.ts';
import { Parser } from './src/utils/kanade/parser.ts';
import { KanadeInterpreter as Interpreter } from './src/utils/kanade/interpreter.ts';
import { registerStandardLibrary } from './src/utils/kanade/stdlib.ts';
import { JapaneseConfig } from './src/utils/kanade/config.ts';
import { localize, syntaxError } from './src/utils/kanade/errs.ts';

const DOCS_DIR = './src/pages/docs';

// 난수와 현재 시각을 쓰는 예제는 실행마다 결과가 달라서, 에러 없이 끝나는지만 본다.
// [파일] 같은 NativeOnly 모듈을 쓰는 예제는 브라우저 엔진이 전용 에러로 거절하는 게 맞다.
const NATIVE_ONLY = /【ファイル】|【ソケット】|【HTTP】/;

const NONDETERMINISTIC = /【乱数】|〈今〉/;
// HANA_BIN overrides where the hana binary is; by default it sits in the sibling ../hana folder.
const GO_EXECUTABLE = process.env.HANA_BIN ?? (process.platform === 'win32' ? '..\\hana\\hana.exe' : '../hana/hana');

function extractKanadeBlocks(markdown: string): string[] {
    const blocks: string[] = [];
    const regex = /```kanade([^\n]*)\n([\s\S]*?)```/g;
    let match;
    while ((match = regex.exec(markdown)) !== null) {
        if (!match[1].includes('skip')) blocks.push(match[2]); // `skip`: an example that needs a peer, only shown
    }
    return blocks;
}

// 에러도 비교한다: 실행이 에러로 끝나면 그 문구(현지화된 최종 메시지)를 출력 뒤에
// 붙여서, TS 엔진이 Go 엔진과 같은 문구를 내는지 잡아낸다. Go 쪽은 stderr의
// "ランタイムエラー: <メッセージ>" 줄에서 같은 메시지를 뽑는다.
async function runTypeScriptEngine(code: string, stdin = ''): Promise<string> {
    let output = "";
    let error = "";
    try {
        const lexer = new Lexer(code);
        const parser = new Parser(lexer.tokens);
        const ast = parser.parseProgram();
        const lines = stdin === '' ? [] : stdin.replace(/\n$/, '').split('\n');
        const interpreter = new Interpreter(ast, async () => lines.shift() ?? "");
        registerStandardLibrary(interpreter);

        interpreter.outputCallback = (msg) => {
            output += msg;
        };
        await interpreter.run();
    } catch (e: any) {
        error = String(e?.message ?? e);
    }
    return withError(output.trim(), error);
}

function withError(output: string, error: string): string {
    return error ? `${output}\n!! ${error}` : output;
}

function runGoEngine(code: string, stdin = ''): string {
    // .knd 확장자여야 hana.exe가 Kanade 렉서/파서로 실행한다(cmd/run.go의
    // isKanade := strings.HasSuffix(filename, ".knd")).
    const tempFile = 'temp_test.knd';
    writeFileSync(tempFile, code);

    try {
        const result = execSync(`${GO_EXECUTABLE} run ${tempFile}`, { input: stdin, stdio: ['pipe', 'pipe', 'pipe'], timeout: 60000 });
        unlinkSync(tempFile);
        return result.toString().trim();
    } catch (e: any) {
        unlinkSync(tempFile);
        const stdout = e.stdout ? e.stdout.toString().trim() : "";
        const stderr = e.stderr ? e.stderr.toString() : "";
        const m = stderr.match(/^ランタイムエラー: (.*)$/m);
        return withError(stdout, m ? m[1].trim() : "");
    }
}

async function walk(dir: string, callback: (path: string) => Promise<void>) {
    const files = readdirSync(dir);
    for (const file of files) {
        const path = join(dir, file);
        if (statSync(path).isDirectory()) {
            await walk(path, callback);
        } else if (path.endsWith('.md') || path.endsWith('.mdx')) {
            await callback(path);
        }
    }
}

const BROWSER_ONLY_CASES: { name: string; code: string; wantError: string }[] = [
    { name: "파일 모듈은 브라우저에서 못 씀", code: "【ファイル】から〈読む〉を持ってこよう\n〈読む〉(「a.txt」)を出力しよう", wantError: "ブラウザでは使えません" },
    { name: "표준: 소켓은 브라우저에서 쓸 수 없어요", code: "【ソケット】から〈接続〉を持ってこよう\n〈接続〉(「127.0.0.1」, 80)を出力しよう\n", wantError: "ブラウザでは使えません" },
    { name: "표준: HTTP는 브라우저에서 쓸 수 없어요", code: "【HTTP】から〈取得〉を持ってこよう\n〈取得〉(「http://127.0.0.1/」)を出力しよう\n", wantError: "ブラウザでは使えません" },
];

// 문서 예제에는 없지만 두 엔진이 같아야 하는 동작: 표준 라이브러리(std) 임포트.
const EXTRA_CASES: { name: string; code: string; stdin?: string }[] = [
    { name: "std 임포트", code: "【数学】から〈切り上げ〉を持ってこよう\n枠「{〈切り上げ〉(3.2)}」を出力しよう" },
    { name: "std 임포트 (별칭)", code: "【数学】から〈切り捨て〉を〈床〉に持ってこよう\n枠「{〈床〉(3.9)}」を出力しよう" },
    { name: "없는 모듈", code: "【なにもない】から〈関数〉を持ってこよう" },
    { name: "멤버 키 오류: 문자열", code: "『x』を「かな」にしよう\n(『x』の『ない』)を出力しよう\n" },
    { name: "멤버 키 오류: 목록 쓰기", code: "『x』を【1, 2】にしよう\n『x』の『ない』を9にしよう\n" },
    { name: "최상위 돌려주자: 프로그램 끝", code: "「前」を出力しよう\n1を返そう\n「後」を出力しよう\n" },
    { name: "타입 이름 값: 설계와 기본 타입", code: "【動物】を設計しよう:\n    『名前』を1にしよう\n【動物】を出力しよう\n(【数字】 == 「数字」)を出力しよう\n『文字列』を5にしよう\n【文字列】を出力しよう\n" },
    { name: "비우기: 설계의 메서드와 문자열", code: "【箱】を設計しよう:\n    『量』を3にしよう\n    〈空にする〉を作ろう():\n        『私』の『量』を0にしよう\n『箱』を新しい【箱】()にしよう\n『箱』の〈空にする〉()を実行しよう\n(『箱』の『量』)を出力しよう\n「字」の〈空にする〉()を実行しよう\n" },
    { name: "비우기: 인자 개수", code: "『目』を【1】にしよう\n『目』の〈空にする〉(1)を実行しよう\n" },
    { name: "없는 함수", code: "【数学】から〈ない関数〉を持ってこよう" },
    { name: "끝없는 재귀", code: "〈f〉を作ろう ():\n    〈f〉()を実行しよう\n〈f〉()を実行しよう" },
    { name: "입력 (문자열)", code: "『名前』を【文字列】で入力してもらおう\n『名前』を出力しよう", stdin: "田中\n" },
    { name: "입력 (숫자, 공백 허용)", code: "『年齢』を【数字】で入力してもらおう\n枠「{『年齢』 + 1}」を出力しよう", stdin: " 20 \n" },
    { name: "입력 (논리)", code: "『はい』を【論理】で入力してもらおう\n『はい』を出力しよう", stdin: "真\n" },
    { name: "입력 (숫자 실패)", code: "『年齢』を【数字】で入力してもらおう\n『年齢』を出力しよう", stdin: "二十\n" },
    { name: "입력 (빈 입력은 숫자가 아님)", code: "『年齢』を【数字】で入力してもらおう\n『年齢』を出力しよう", stdin: "" },
    { name: "타입: 선언 불일치", code: "『a』を【数字】の「文字」にしよう" },
    { name: "타입: 재대입 불일치", code: "『a』を【数字】の3にしよう\n『a』を「x」にしよう" },
    { name: "타입: 비어있음 허용", code: "『a』を【数字】の3にしよう\n『a』を空っぽにしよう\n『a』を出力しよう" },
    { name: "타입: 제네릭 push", code: "『目録』を【(数字)リスト】の【1,2】にしよう\n『目録』の後に「三」を追加しよう" },
    { name: "타입: 제네릭 사전", code: "『点数』を【(文字列,数字)辞書】の{「国語」:「九十」}にしよう" },
    { name: "타입: 매개변수", code: "〈二倍〉を作ろう(【数字】の『値』):\n    (『値』 * 2)を出力しよう\n〈二倍〉(「文字」)を実行しよう" },
    { name: "타입: 반환 통과", code: "【数字】を返す〈二乗〉を作ろう(【数字】の『x』):\n    (『x』 * 『x』)を返そう\n〈二乗〉(4)を出力しよう" },
    { name: "표준: CSV 파싱", code: "【CSV】から〈パース〉を持ってこよう\n【CSV】から〈文字列化〉を持ってこよう\n〈パース〉(「a,b\\nc,d\\n」)を出力しよう" },
    { name: "표준: CSV 따옴표와 쉼표", code: "【CSV】から〈パース〉を持ってこよう\n【CSV】から〈文字列化〉を持ってこよう\n〈パース〉(「\"x,y\",\"a\"\"b\"」)を出力しよう" },
    { name: "표준: CSV 구분자", code: "【CSV】から〈パース〉を持ってこよう\n【CSV】から〈文字列化〉を持ってこよう\n〈パース〉(「a;b」, 「;」)を出力しよう" },
    { name: "표준: CSV 문자열화", code: "【CSV】から〈パース〉を持ってこよう\n【CSV】から〈文字列化〉を持ってこよう\n〈文字列化〉(【【「a,b」, 1, 2.5】, 【「」】】)を出力しよう" },
    { name: "표준: CSV 닫히지 않은 따옴표", code: "【CSV】から〈パース〉を持ってこよう\n【CSV】から〈文字列化〉を持ってこよう\n〈パース〉(「\"abc」)を出力しよう" },
    { name: "연산: 우선순위", code: "(2 + 3 * 4)を出力しよう\n(10 - 4 / 2)を出力しよう\n(2 * 3 + 4 * 5)を出力しよう\n(100 / 10 / 5)を出力しよう\n(10 - 3 - 2)を出力しよう\n(7 % 4 + 1)を出力しよう\n((2 + 3) * 4)を出力しよう" },
    { name: "타입: 반환 불일치", code: "【数字】を返す〈違う〉を作ろう():\n    「文字」を返そう\n〈違う〉()を出力しよう" },
    { name: "타입: 업캐스팅", code: "【動物】を設計しよう:\n    『名前』を【文字列】の「動物」にしよう\n【動物】をもとにして【子犬】を設計しよう:\n    『芸』を【文字列】の「おすわり」にしよう\n【猫】を設計しよう:\n    『名前』を【文字列】の「ミケ」にしよう\n『友達』を【動物】の新しい【子犬】()にしよう\n『友達』の『芸』を出力しよう" },
    { name: "타입: 낯선 클래스", code: "【動物】を設計しよう:\n    『名前』を【文字列】の「動物」にしよう\n【動物】をもとにして【子犬】を設計しよう:\n    『芸』を【文字列】の「おすわり」にしよう\n【猫】を設計しよう:\n    『名前』を【文字列】の「ミケ」にしよう\n『友達』を【動物】の新しい【猫】()にしよう" },
    { name: "타입: 필드 쓰기", code: "【人】を設計しよう:\n    『年齢』を【数字】の20にしよう\n『太郎』を【人】の新しい【人】()にしよう\n『太郎』の『年齢』を「二十」にしよう" },
    { name: "연산: 숫자 + 문자열", code: "(10 + 「こんにちは」)を出力しよう" },
    { name: "연산: 문자열 - 문자열", code: "(「あ」 - 「い」)を出力しよう" },
    { name: "연산: 문자열 + 문자열", code: "(「あ」 + 「い」)を出力しよう" },
    { name: "연산: 비어있음 피연산자", code: "(1 + 空っぽ)を出力しよう" },
    { name: "연산: 비어있음 동등 비교", code: "(空っぽ == 空っぽ)を出力しよう" },
    { name: "연산: 복합 대입", code: "『a』を「x」にしよう\n『a』に3を足そう" },
    { name: "추상: 인터페이스 생성", code: "【飛べるもの】を規定しよう:\n    〈飛ぶ〉がなければならない()\n新しい【飛べるもの】()を実行しよう\n「作られた」を出力しよう" },
    { name: "추상: 추상 클래스 생성", code: "【図形】を下設計しよう:\n    〈面積〉を作ろう():\n        1を出力しよう\n新しい【図形】()を実行しよう\n「作られた」を出力しよう" },
    { name: "추상: 자식 클래스는 생성 가능", code: "【図形】を下設計しよう:\n    〈面積〉がなければならない()\n【図形】をもとにして【四角形】を設計しよう:\n    【数字】を返す〈面積〉を作ろう():\n        6を返そう\n『形』を【図形】の新しい【四角形】()にしよう\n(『形』の〈面積〉())を出力しよう" },
    { name: "불변: 문자열 글자 재대입", code: "『名前』を「山田」にしよう\n『名前』の1番目を「田」にしよう\n『名前』を出力しよう" },
    { name: "표준: JSON 파싱", code: "【JSON】から〈パース〉を持ってこよう\n【JSON】から〈文字列化〉を持ってこよう\n『値』を〈パース〉(「{\"a\": [1, {\"b\": true}], \"c\": null}」)にしよう\n『値』を出力しよう" },
    { name: "표준: JSON 문자열화 키 정렬", code: "【JSON】から〈パース〉を持ってこよう\n【JSON】から〈文字列化〉を持ってこよう\n〈文字列化〉({「b」: 1, 「a」: 【真, 空っぽ, 「x」】})を出力しよう" },
    { name: "표준: JSON 들여쓰기", code: "【JSON】から〈パース〉を持ってこよう\n【JSON】から〈文字列化〉を持ってこよう\n〈文字列化〉({「a」: 【1, 2】, 「b」: {}}, 2)を出力しよう" },
    { name: "표준: JSON 숫자 형식", code: "【JSON】から〈パース〉を持ってこよう\n【JSON】から〈文字列化〉を持ってこよう\n〈文字列化〉(【0.5, 100, 2.25, 0.0000001, 1000000000000000000000】)を出力しよう" },
    { name: "표준: JSON 왕복", code: "【JSON】から〈パース〉を持ってこよう\n【JSON】から〈文字列化〉を持ってこよう\n〈パース〉(〈文字列化〉({「名前」: 「はな」}))を出力しよう" },
    { name: "표준: JSON 잘못된 입력", code: "【JSON】から〈パース〉を持ってこよう\n【JSON】から〈文字列化〉を持ってこよう\n〈パース〉(「{」)を出力しよう" },
    { name: "표준: JSON 문자열 아닌 키", code: "【JSON】から〈パース〉を持ってこよう\n【JSON】から〈文字列化〉を持ってこよう\n〈文字列化〉({1: 2})を出力しよう" },
    { name: "표준: JSON 범위 밖 숫자", code: "【JSON】から〈パース〉を持ってこよう\n【JSON】から〈文字列化〉を持ってこよう\n〈パース〉(「1e999」)を出力しよう" },
    { name: "표준: 무작위 정수 범위", code: "【乱数】から〈実数〉を持ってこよう\n【乱数】から〈整数〉を持ってこよう\n【乱数】から〈選ぶ〉を持ってこよう\n【乱数】から〈シャッフル〉を持ってこよう\n『値』を〈整数〉(3, 5)にしよう\n(『値』 >= 3)を出力しよう" },
    { name: "표준: 무작위 정수 뒤집힘", code: "【乱数】から〈実数〉を持ってこよう\n【乱数】から〈整数〉を持ってこよう\n【乱数】から〈選ぶ〉を持ってこよう\n【乱数】から〈シャッフル〉を持ってこよう\n〈整数〉(3, 1)を出力しよう" },
    { name: "표준: 무작위 정수 소수", code: "【乱数】から〈実数〉を持ってこよう\n【乱数】から〈整数〉を持ってこよう\n【乱数】から〈選ぶ〉を持ってこよう\n【乱数】から〈シャッフル〉を持ってこよう\n〈整数〉(1.5, 3)を出力しよう" },
    { name: "표준: 무작위 빈 목록", code: "【乱数】から〈実数〉を持ってこよう\n【乱数】から〈整数〉を持ってこよう\n【乱数】から〈選ぶ〉を持ってこよう\n【乱数】から〈シャッフル〉を持ってこよう\n『空』を【(何でも)リスト】の【】にしよう\n〈選ぶ〉(『空』)を出力しよう" },
    { name: "표준: 무작위 섞기 길이", code: "【乱数】から〈実数〉を持ってこよう\n【乱数】から〈整数〉を持ってこよう\n【乱数】から〈選ぶ〉を持ってこよう\n【乱数】から〈シャッフル〉を持ってこよう\n『元』を【1, 2, 3, 4】にしよう\n『混ぜた』を〈シャッフル〉(『元』)にしよう\n『混ぜた』の『長さ』を出力しよう\n『元』を出力しよう" },
    { name: "표준: 날짜 왕복", code: "【日時】から〈書式〉を持ってこよう\n【日時】から〈読み取り〉を持ってこよう\n〈書式〉(〈読み取り〉(「2024-03-05 07:08:09」, 「YYYY-MM-DD HH:mm:ss」), 「YYYY/MM/DD HH:mm:ss」)を出力しよう" },
    { name: "표준: 날짜 서식 글자", code: "【日時】から〈書式〉を持ってこよう\n【日時】から〈読み取り〉を持ってこよう\n〈書式〉(〈読み取り〉(「2024-03-05」, 「YYYY-MM-DD」), 「YYYY年MM月DD日」)を出力しよう" },
    { name: "표준: 날짜 없는 날", code: "【日時】から〈書式〉を持ってこよう\n【日時】から〈読み取り〉を持ってこよう\n〈読み取り〉(「2024-02-30」, 「YYYY-MM-DD」)を出力しよう" },
    { name: "표준: 날짜 서식 불일치", code: "【日時】から〈書式〉を持ってこよう\n【日時】から〈読み取り〉を持ってこよう\n〈読み取り〉(「2024/03/05」, 「YYYY-MM-DD」)を出力しよう" },
    { name: "표준: 날짜 부호", code: "【日時】から〈書式〉を持ってこよう\n【日時】から〈読み取り〉を持ってこよう\n〈読み取り〉(「2024-+3-05」, 「YYYY-MM-DD」)を出力しよう" },
    { name: "표준: 정규식 일본어", code: "【正規表現】から〈検査〉を持ってこよう\n【正規表現】から〈検索〉を持ってこよう\n【正規表現】から〈グループ〉を持ってこよう\n【正規表現】から〈全検索〉を持ってこよう\n【正規表現】から〈置換〉を持ってこよう\n【正規表現】から〈分割〉を持ってこよう\n〈検索〉(「abcひらがなdef」, 「[ぁ-ん]+」)を出力しよう" },
    { name: "표준: 정규식 없음", code: "【正規表現】から〈検査〉を持ってこよう\n【正規表現】から〈検索〉を持ってこよう\n【正規表現】から〈グループ〉を持ってこよう\n【正規表現】から〈全検索〉を持ってこよう\n【正規表現】から〈置換〉を持ってこよう\n【正規表現】から〈分割〉を持ってこよう\n〈検索〉(「abc」, 「[0-9]+」)を出力しよう" },
    { name: "표준: 정규식 그룹", code: "【正規表現】から〈検査〉を持ってこよう\n【正規表現】から〈検索〉を持ってこよう\n【正規表現】から〈グループ〉を持ってこよう\n【正規表現】から〈全検索〉を持ってこよう\n【正規表現】から〈置換〉を持ってこよう\n【正規表現】から〈分割〉を持ってこよう\n〈グループ〉(「b」, 「(a)|(b)」)を出力しよう" },
    { name: "표준: 정규식 치환 그룹", code: "【正規表現】から〈検査〉を持ってこよう\n【正規表現】から〈検索〉を持ってこよう\n【正規表現】から〈グループ〉を持ってこよう\n【正規表現】から〈全検索〉を持ってこよう\n【正規表現】から〈置換〉を持ってこよう\n【正規表現】から〈分割〉を持ってこよう\n〈置換〉(「山田 鈴木」, 「([^ ]+) ([^ ]+)」, 「$2 $1」)を出力しよう" },
    { name: "표준: 정규식 치환 달러", code: "【正規表現】から〈検査〉を持ってこよう\n【正規表現】から〈検索〉を持ってこよう\n【正規表現】から〈グループ〉を持ってこよう\n【正規表現】から〈全検索〉を持ってこよう\n【正規表現】から〈置換〉を持ってこよう\n【正規表現】から〈分割〉を持ってこよう\n〈置換〉(「a1」, 「[0-9]」, 「$x$$」)を出力しよう" },
    { name: "표준: 정규식 없는 그룹", code: "【正規表現】から〈検査〉を持ってこよう\n【正規表現】から〈検索〉を持ってこよう\n【正規表現】から〈グループ〉を持ってこよう\n【正規表現】から〈全検索〉を持ってこよう\n【正規表現】から〈置換〉を持ってこよう\n【正規表現】から〈分割〉を持ってこよう\n〈置換〉(「a1」, 「[0-9]」, 「<$3>」)を出力しよう" },
    { name: "표준: 정규식 분할", code: "【正規表現】から〈検査〉を持ってこよう\n【正規表現】から〈検索〉を持ってこよう\n【正規表現】から〈グループ〉を持ってこよう\n【正規表現】から〈全検索〉を持ってこよう\n【正規表現】から〈置換〉を持ってこよう\n【正規表現】から〈分割〉を持ってこよう\n〈分割〉(「a, b,c」, 「, ?」)を出力しよう" },
    { name: "표준: 정규식 분할 캡처", code: "【正規表現】から〈検査〉を持ってこよう\n【正規表現】から〈検索〉を持ってこよう\n【正規表現】から〈グループ〉を持ってこよう\n【正規表現】から〈全検索〉を持ってこよう\n【正規表現】から〈置換〉を持ってこよう\n【正規表現】から〈分割〉を持ってこよう\n〈分割〉(「a1b2c」, 「([0-9])」)を出力しよう" },
    { name: "표준: 정규식 모두 없음", code: "【正規表現】から〈検査〉を持ってこよう\n【正規表現】から〈検索〉を持ってこよう\n【正規表現】から〈グループ〉を持ってこよう\n【正規表現】から〈全検索〉を持ってこよう\n【正規表現】から〈置換〉を持ってこよう\n【正規表現】から〈分割〉を持ってこよう\n〈全検索〉(「abc」, 「[0-9]」)を出力しよう" },
    { name: "표준: 정규식 모두찾기", code: "【正規表現】から〈検査〉を持ってこよう\n【正規表現】から〈検索〉を持ってこよう\n【正規表現】から〈グループ〉を持ってこよう\n【正規表現】から〈全検索〉を持ってこよう\n【正規表現】から〈置換〉を持ってこよう\n【正規表現】から〈分割〉を持ってこよう\n〈全検索〉(「a1b22c333」, 「[0-9]+」)を出力しよう" },
    { name: "표준: 정규식 잘못된 패턴", code: "【正規表現】から〈検査〉を持ってこよう\n【正規表現】から〈検索〉を持ってこよう\n【正規表現】から〈グループ〉を持ってこよう\n【正規表現】から〈全検索〉を持ってこよう\n【正規表現】から〈置換〉を持ってこよう\n【正規表現】から〈分割〉を持ってこよう\n〈検査〉(「a」, 「(」)を出力しよう" },
    { name: "표준: 정규식 대소문자 플래그", code: "【正規表現】から〈検査〉を持ってこよう\n【正規表現】から〈検索〉を持ってこよう\n【正規表現】から〈グループ〉を持ってこよう\n【正規表現】から〈全検索〉を持ってこよう\n【正規表現】から〈置換〉を持ってこよう\n【正規表現】から〈分割〉を持ってこよう\n〈検査〉(「ABC」, 「(?i)abc」)を出力しよう" },
    { name: "표준: 정규식 인자 종류", code: "【正規表現】から〈検査〉を持ってこよう\n【正規表現】から〈検索〉を持ってこよう\n【正規表現】から〈グループ〉を持ってこよう\n【正規表現】から〈全検索〉を持ってこよう\n【正規表現】から〈置換〉を持ってこよう\n【正規表現】から〈分割〉を持ってこよう\n〈検査〉(1, 「a」)を出力しよう" },
    { name: "표준: 요일 월요일", code: "【日時】から〈読み取り〉を持ってこよう\n【日時】から〈曜日〉を持ってこよう\n〈曜日〉(〈読み取り〉(「2024-03-04」, 「YYYY-MM-DD」))を出力しよう" },
    { name: "표준: 요일 일요일", code: "【日時】から〈読み取り〉を持ってこよう\n【日時】から〈曜日〉を持ってこよう\n〈曜日〉(〈読み取り〉(「2024-03-10」, 「YYYY-MM-DD」))を出力しよう" },
    { name: "표준: 요일 인자 종류", code: "【日時】から〈曜日〉を持ってこよう\n〈曜日〉(「月」)を出力しよう" },
    { name: "가져오기: 여러 개", code: "【数学】から〈切り上げ〉と〈切り捨て〉を持ってこよう\n〈切り上げ〉(3.2)を出力しよう\n〈切り捨て〉(3.8)を出力しよう" },
    { name: "가져오기: 세 개", code: "【正規表現】から〈検査〉と〈検索〉と〈分割〉を持ってこよう\n〈検査〉(「a1」, 「[0-9]」)を出力しよう\n〈検索〉(「a1」, 「[0-9]」)を出力しよう\n〈分割〉(「a,b」, 「,」)を出力しよう" },
    { name: "가져오기: 목록 중 없는 것", code: "【数学】から〈切り上げ〉と〈なし〉を持ってこよう" },
    { name: "가져오기: 별칭 한 개", code: "【数学】から〈切り上げ〉を〈丸め〉に持ってこよう\n〈丸め〉(3.2)を出力しよう" },
    { name: "가져오기: 전부", code: "【数学】から全部持ってこよう\n〈切り上げ〉(3.2)を出力しよう\n〈切り捨て〉(3.8)を出力しよう" },
    { name: "가져오기: 전부 큰 모듈", code: "【日時】から全部持ってこよう\n〈書式〉(〈読み取り〉(「2024-03-05」, 「YYYY-MM-DD」), 「MM/DD」)を出力しよう" },
    { name: "가져오기: 전부 없는 모듈", code: "【なし】から全部持ってこよう" },
    { name: "가져오기: 가져오기 전에는 없음", code: "〈切り上げ〉(3.2)を出力しよう" },
    { name: "표준: 수학: 제곱근", code: "【数学】から〈平方根〉を持ってこよう\n〈平方根〉(2)を出力しよう\n" },
    { name: "표준: 수학: 거듭제곱", code: "【数学】から〈べき乗〉を持ってこよう\n〈べき乗〉(2, 10)を出力しよう\n" },
    { name: "표준: 수학: 반올림", code: "【数学】から〈四捨五入〉を持ってこよう\n〈四捨五入〉(2.5)を出力しよう\n〈四捨五入〉((0 - 2.5))を出力しよう\n〈四捨五入〉(1234.5678, 2)を出力しよう\n〈四捨五入〉((0 - 0.4))を出力しよう\n" },
    { name: "표준: 수학: 사인 코사인 로그", code: "【数学】から〈サイン〉を持ってこよう\n【数学】から〈コサイン〉を持ってこよう\n【数学】から〈対数〉を持ってこよう\n【数学】から〈タンジェント〉を持ってこよう\n〈サイン〉(0)を出力しよう\n〈コサイン〉(0)を出力しよう\n〈対数〉(1)を出力しよう\n〈タンジェント〉(0)を出力しよう\n" },
    { name: "표준: 수학: 최대공약수 팩토리얼 절댓값", code: "【数学】から〈最大公約数〉を持ってこよう\n【数学】から〈階乗〉を持ってこよう\n【数学】から〈絶対値〉を持ってこよう\n〈最大公約数〉((0 - 12), 18)を出力しよう\n〈階乗〉(10)を出力しよう\n〈階乗〉(170)を出力しよう\n〈絶対値〉((0 - 7))を出力しよう\n" },
    { name: "표준: 수학: 음수 제곱근", code: "【数学】から〈平方根〉を持ってこよう\n〈平方根〉((0 - 1))を出力しよう\n" },
    { name: "표준: 수학: 로그 0", code: "【数学】から〈対数〉を持ってこよう\n〈対数〉(0)を出力しよう\n" },
    { name: "표준: 수학: 넘치는 거듭제곱", code: "【数学】から〈べき乗〉を持ってこよう\n〈べき乗〉(10, 400)を出力しよう\n" },
    { name: "표준: 수학: 팩토리얼 소수", code: "【数学】から〈階乗〉を持ってこよう\n〈階乗〉(2.5)を出力しよう\n" },
    { name: "표준: 통계: 기본", code: "【統計】から〈合計〉を持ってこよう\n【統計】から〈最小値〉を持ってこよう\n【統計】から〈最大値〉を持ってこよう\n【統計】から〈平均〉を持ってこよう\n【統計】から〈中央値〉を持ってこよう\n【統計】から〈標準偏差〉を持ってこよう\n〈合計〉(【1, 2, 3.5】)を出力しよう\n〈最小値〉(【3, 1, 2】)を出力しよう\n〈最大値〉(【3, 1, 2】)を出力しよう\n〈平均〉(【1, 2, 3, 4】)を出力しよう\n〈中央値〉(【4, 1, 3, 2】)を出力しよう\n〈中央値〉(【5, 1, 3】)を出力しよう\n〈標準偏差〉(【2, 4, 4, 4, 5, 5, 7, 9】)を出力しよう\n〈合計〉(【】)を出力しよう\n" },
    { name: "표준: 통계: 빈 목록 평균", code: "【統計】から〈平均〉を持ってこよう\n〈平均〉(【】)を出力しよう\n" },
    { name: "표준: 통계: 표준편차 한 개", code: "【統計】から〈標準偏差〉を持ってこよう\n〈標準偏差〉(【5】)を出力しよう\n" },
    { name: "표준: 통계: 숫자 아닌 값", code: "【統計】から〈最大値〉を持ってこよう\n〈最大値〉(【1, 「二」】)を出力しよう\n" },
    { name: "표준: 텍스트: 대소문자 다듬기", code: "【テキスト】から〈大文字〉を持ってこよう\n【テキスト】から〈小文字〉を持ってこよう\n【テキスト】から〈トリム〉を持ってこよう\n〈大文字〉(「hello ひらがな ß」)を出力しよう\n〈小文字〉(「ABC ひらがな」)を出力しよう\n枠「[{〈トリム〉(「　 あ い 　」)}]」を出力しよう\n" },
    { name: "표준: 텍스트: 채우기 반복 거꾸로", code: "【テキスト】から〈左埋め〉を持ってこよう\n【テキスト】から〈右埋め〉を持ってこよう\n【テキスト】から〈繰り返し〉を持ってこよう\n【テキスト】から〈逆さ〉を持ってこよう\n〈左埋め〉(「あ」, 3, 「0」)を出力しよう\n〈右埋め〉(「あいうえ」, 3, 「0」)を出力しよう\n〈繰り返し〉(「ab」, 3)を出力しよう\n〈逆さ〉(「あいう」)を出力しよう\n" },
    { name: "표준: 텍스트: 채울 글자 두 개", code: "【テキスト】から〈左埋め〉を持ってこよう\n〈左埋め〉(「7」, 3, 「ab」)を出力しよう\n" },
    { name: "표준: 텍스트: 너무 큰 반복", code: "【テキスト】から〈繰り返し〉を持ってこよう\n〈繰り返し〉(「abcdefghij」, 200000)を出力しよう\n" },
    { name: "표준: 텍스트: 음수 반복", code: "【テキスト】から〈繰り返し〉を持ってこよう\n〈繰り返し〉(「ab」, (0 - 1))を出力しよう\n" },
    { name: "표준: 텍스트: 시작 끝 잇기 세기 위치", code: "【テキスト】から〈始まるか〉を持ってこよう\n【テキスト】から〈終わるか〉を持ってこよう\n【テキスト】から〈連結〉を持ってこよう\n【テキスト】から〈数える〉を持ってこよう\n【テキスト】から〈位置〉を持ってこよう\n〈始まるか〉(「abc」, 「」)を出力しよう\n〈終わるか〉(「hello」, 「lo」)を出力しよう\n〈連結〉(【「あ」, 「い」, 「う」】, 「、」)を出力しよう\n〈数える〉(「aaaa」, 「aa」)を出力しよう\n〈位置〉(「あいう」, 「う」)を出力しよう\n〈位置〉(「あいう」, 「え」)を出力しよう\n" },
    { name: "표준: 텍스트: 빈 것 세기", code: "【テキスト】から〈数える〉を持ってこよう\n〈数える〉(「abc」, 「」)を出力しよう\n" },
    { name: "표준: 텍스트: 문자열 아닌 잇기", code: "【テキスト】から〈連結〉を持ってこよう\n〈連結〉(【1, 2】, 「,」)を出力しよう\n" },
    { name: "표준: 목록: 정렬 뒤집기 중복제거", code: "【リスト】から〈並べ替え〉を持ってこよう\n【リスト】から〈反転〉を持ってこよう\n【リスト】から〈重複除去〉を持ってこよう\n『元』を【「う」, 「あ」, 「い」】にしよう\n〈並べ替え〉(『元』)を出力しよう\n『元』を出力しよう\n〈並べ替え〉(【3, 1, 2】)を出力しよう\n〈並べ替え〉(【】)を出力しよう\n〈反転〉(【1, 2, 3】)を出力しよう\n〈重複除去〉(【3, 1, 3, 「1」, 1, 「1」】)を出力しよう\n" },
    { name: "표준: 목록: 섞인 정렬", code: "【リスト】から〈並べ替え〉を持ってこよう\n〈並べ替え〉(【1, 「二」】)を出力しよう\n" },
    { name: "표준: 목록: 목록 중복제거", code: "【リスト】から〈重複除去〉を持ってこよう\n〈重複除去〉(【【1】, 【1】】)を出力しよう\n" },
    { name: "표준: 목록: 범위", code: "【リスト】から〈範囲〉を持ってこよう\n〈範囲〉(5, 1)を出力しよう\n〈範囲〉(1, 10, 4)を出力しよう\n〈範囲〉(1, 5, (0 - 1))を出力しよう\n〈範囲〉(3, 3)を出力しよう\n" },
    { name: "표준: 목록: 간격 0", code: "【リスト】から〈範囲〉を持ってこよう\n〈範囲〉(1, 5, 0)を出力しよう\n" },
    { name: "표준: 목록: 너무 긴 범위", code: "【リスト】から〈範囲〉を持ってこよう\n〈範囲〉(1, 2000000)を出力しよう\n" },
    { name: "표준: 목록: 평탄화 조각내기 짝짓기", code: "【リスト】から〈平坦化〉を持ってこよう\n【リスト】から〈小分け〉を持ってこよう\n【リスト】から〈ペア〉を持ってこよう\n〈平坦化〉(【1, 【2, 【3】】, 4】)を出力しよう\n〈小分け〉(【1, 2】, 5)を出力しよう\n〈小分け〉(【1, 2, 3】, 2)を出力しよう\n〈ペア〉(【1, 2, 3】, 【「あ」, 「い」】)を出力しよう\n" },
    { name: "표준: 목록: 조각 0", code: "【リスト】から〈小分け〉を持ってこよう\n〈小分け〉(【1, 2】, 0)を出力しよう\n" },
    { name: "표준: 인코딩: 베이스64", code: "【エンコード】から〈Base64エンコード〉を持ってこよう\n【エンコード】から〈Base64デコード〉を持ってこよう\n〈Base64エンコード〉(「日本」)を出力しよう\n〈Base64デコード〉(「5pel5pys」)を出力しよう\n〈Base64エンコード〉(「」)を出力しよう\n" },
    { name: "표준: 인코딩: 패딩 없는 베이스64", code: "【エンコード】から〈Base64デコード〉を持ってこよう\n〈Base64デコード〉(「aGk」)を出力しよう\n" },
    { name: "표준: 인코딩: 글자가 아닌 바이트", code: "【エンコード】から〈Base64デコード〉を持ってこよう\n〈Base64デコード〉(「/w==」)を出力しよう\n" },
    { name: "표준: 인코딩: 주소", code: "【エンコード】から〈URLエンコード〉を持ってこよう\n【エンコード】から〈URLデコード〉を持ってこよう\n〈URLエンコード〉(「あ a+b~」)を出力しよう\n〈URLデコード〉(「%E3%81%82%20a%2Bb」)を出力しよう\n〈URLデコード〉(「a+b」)を出力しよう\n" },
    { name: "표준: 인코딩: 깨진 주소", code: "【エンコード】から〈URLデコード〉を持ってこよう\n〈URLデコード〉(「%2」)を出力しよう\n" },
    { name: "표준: 인코딩: 16진수 아닌 주소", code: "【エンコード】から〈URLデコード〉を持ってこよう\n〈URLデコード〉(「%zz」)を出力しよう\n" },
    { name: "표준: 해시: SHA256", code: "【ハッシュ】から〈SHA256〉を持ってこよう\n〈SHA256〉(「」)を出力しよう\n〈SHA256〉(「abc」)を出力しよう\n〈SHA256〉(「日本」)を出力しよう\n〈SHA256〉(「aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa」)を出力しよう\n" },
    { name: "표준: 고유번호 모양", code: "【乱数】から〈UUID〉を持ってこよう\n【正規表現】から〈検査〉を持ってこよう\n〈検査〉(〈UUID〉(), 「^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$」)を出力しよう\n" },
    { name: "표준: 기다리기", code: "【日時】から〈待つ〉を持ってこよう\n〈待つ〉(0.05)を実行しよう\n「終」を出力しよう" },
    { name: "표준: 기다리기 음수", code: "【日時】から〈待つ〉を持ってこよう\n〈待つ〉((0 - 1))を実行しよう" },
    { name: "표준: 기다리기 너무 김", code: "【日時】から〈待つ〉を持ってこよう\n〈待つ〉(3601)を実行しよう" },
    { name: "표준: 기다리기 인자 종류", code: "【日時】から〈待つ〉を持ってこよう\n〈待つ〉(「少し」)を実行しよう" },
    { name: "표준: 경로", code: "【パス】から全部持ってこよう\n〈結合〉(「a」, 「b」, 「c.txt」)を出力しよう\n〈結合〉(「a/」, 「/b」, 「c」)を出力しよう\n〈結合〉(「a」, 「」, 「b」)を出力しよう\n〈結合〉(「C:\\\\data」, 「x.txt」)を出力しよう\n〈フォルダ名〉(「a/b/c.txt」)を出力しよう\n〈フォルダ名〉(「c.txt」)を出力しよう\n〈フォルダ名〉(「/」)を出力しよう\n〈ファイル名〉(「a/b/」)を出力しよう\n〈ファイル名〉(「日本語/ファイル.txt」)を出力しよう\n〈拡張子〉(「a/b.tar.gz」)を出力しよう\n〈拡張子〉(「a/.bashrc」)を出力しよう\n〈拡張子なし名〉(「a/b.tar.gz」)を出力しよう\n〈拡張子変更〉(「a/b.txt」, 「md」)を出力しよう\n〈拡張子変更〉(「a/b.txt」, 「」)を出力しよう\n〈正規化〉(「a//b/./c/../d」)を出力しよう\n〈正規化〉(「../a/../..」)を出力しよう\n〈正規化〉(「」)を出力しよう\n〈絶対パスか〉(「/a」)を出力しよう\n〈絶対パスか〉(「C:a」)を出力しよう\n〈構成要素〉(「/a/b」)を出力しよう\n〈構成要素〉(「a/./b//c/..」)を出力しよう\n" },
    { name: "표준: 경로 인자 오류 1", code: "【パス】から〈結合〉を持ってこよう\n〈結合〉()を出力しよう" },
    { name: "표준: 경로 인자 오류 2", code: "【パス】から〈ファイル名〉を持ってこよう\n〈ファイル名〉(1)を出力しよう" },
    { name: "표준: 목록 콜백 기본", code: "【リスト】から全部持ってこよう\n〈二倍〉を作ろう(『x』):\n    (『x』 * 2)を返そう\n〈偶数〉を作ろう(『x』):\n    もし(『x』 % 2 == 0)なら:\n        真を返そう\n    偽を返そう\n〈足す〉を作ろう(『合計』, 『x』):\n    (『合計』 + 『x』)を返そう\n〈自分〉を作ろう(『x』):\n    『x』を返そう\n〈数字ではない〉を作ろう(『x』):\n    「はい」を返そう\n〈壊れた〉を作ろう(『x』):\n    新しい【エラー】(「コールバックが壊れました」)を発生させよう\n〈長さ〉を作ろう(『文』):\n    『文』の『長さ』を返そう\n〈何もしない〉を作ろう(『x』):\n    『y』を【数字】の1にしよう\n〈変換〉(【1, 2, 3】, 〈二倍〉)を出力しよう\n〈絞り込み〉(【1, 2, 3, 4】, 〈偶数〉)を出力しよう\n〈畳み込み〉(【1, 2, 3, 4】, 〈足す〉, 0)を出力しよう\n〈畳み込み〉(【「あ」, 「い」, 「う」】, 〈足す〉, 「」)を出力しよう\n〈探す〉(【1, 3, 4, 6】, 〈偶数〉)を出力しよう\n〈探す〉(【1, 3】, 〈偶数〉)を出力しよう\n〈どれか〉(【1, 3, 4】, 〈偶数〉)を出力しよう\n〈どれか〉(【1, 3】, 〈偶数〉)を出力しよう\n〈すべて〉(【2, 4】, 〈偶数〉)を出力しよう\n〈すべて〉(【2, 3】, 〈偶数〉)を出力しよう\n〈変換〉(【1, 2】, 〈何もしない〉)を出力しよう\n" },
    { name: "표준: 목록 콜백 빈 목록", code: "【リスト】から全部持ってこよう\n〈二倍〉を作ろう(『x』):\n    (『x』 * 2)を返そう\n〈偶数〉を作ろう(『x』):\n    もし(『x』 % 2 == 0)なら:\n        真を返そう\n    偽を返そう\n〈足す〉を作ろう(『合計』, 『x』):\n    (『合計』 + 『x』)を返そう\n〈自分〉を作ろう(『x』):\n    『x』を返そう\n〈数字ではない〉を作ろう(『x』):\n    「はい」を返そう\n〈壊れた〉を作ろう(『x』):\n    新しい【エラー】(「コールバックが壊れました」)を発生させよう\n〈長さ〉を作ろう(『文』):\n    『文』の『長さ』を返そう\n〈何もしない〉を作ろう(『x』):\n    『y』を【数字】の1にしよう\n〈変換〉(【】, 〈二倍〉)を出力しよう\n〈絞り込み〉(【】, 〈偶数〉)を出力しよう\n〈畳み込み〉(【】, 〈足す〉, 7)を出力しよう\n〈探す〉(【】, 〈偶数〉)を出力しよう\n〈どれか〉(【】, 〈偶数〉)を出力しよう\n〈すべて〉(【】, 〈偶数〉)を出力しよう\n〈基準並べ替え〉(【】, 〈二倍〉)を出力しよう\n" },
    { name: "표준: 목록 콜백 기준정렬", code: "【リスト】から全部持ってこよう\n〈二倍〉を作ろう(『x』):\n    (『x』 * 2)を返そう\n〈偶数〉を作ろう(『x』):\n    もし(『x』 % 2 == 0)なら:\n        真を返そう\n    偽を返そう\n〈足す〉を作ろう(『合計』, 『x』):\n    (『合計』 + 『x』)を返そう\n〈自分〉を作ろう(『x』):\n    『x』を返そう\n〈数字ではない〉を作ろう(『x』):\n    「はい」を返そう\n〈壊れた〉を作ろう(『x』):\n    新しい【エラー】(「コールバックが壊れました」)を発生させよう\n〈長さ〉を作ろう(『文』):\n    『文』の『長さ』を返そう\n〈何もしない〉を作ろう(『x』):\n    『y』を【数字】の1にしよう\n〈基準並べ替え〉(【「ab」, 「あ」, 「ううう」】, 〈長さ〉)を出力しよう\n〈基準並べ替え〉(【「b」, 「a」, 「c」】, 〈自分〉)を出力しよう\n〈基準並べ替え〉(【3, 1, 2】, 〈自分〉)を出力しよう\n〈基準並べ替え〉(【「bb」, 「a」, 「cc」, 「d」】, 〈長さ〉)を出力しよう\n" },
    { name: "표준: 목록 콜백 오류 1", code: "【リスト】から全部持ってこよう\n〈二倍〉を作ろう(『x』):\n    (『x』 * 2)を返そう\n〈偶数〉を作ろう(『x』):\n    もし(『x』 % 2 == 0)なら:\n        真を返そう\n    偽を返そう\n〈足す〉を作ろう(『合計』, 『x』):\n    (『合計』 + 『x』)を返そう\n〈自分〉を作ろう(『x』):\n    『x』を返そう\n〈数字ではない〉を作ろう(『x』):\n    「はい」を返そう\n〈壊れた〉を作ろう(『x』):\n    新しい【エラー】(「コールバックが壊れました」)を発生させよう\n〈長さ〉を作ろう(『文』):\n    『文』の『長さ』を返そう\n〈何もしない〉を作ろう(『x』):\n    『y』を【数字】の1にしよう\n〈絞り込み〉(【1, 2】, 〈数字ではない〉)を出力しよう\n" },
    { name: "표준: 목록 콜백 오류 2", code: "【リスト】から全部持ってこよう\n〈二倍〉を作ろう(『x』):\n    (『x』 * 2)を返そう\n〈偶数〉を作ろう(『x』):\n    もし(『x』 % 2 == 0)なら:\n        真を返そう\n    偽を返そう\n〈足す〉を作ろう(『合計』, 『x』):\n    (『合計』 + 『x』)を返そう\n〈自分〉を作ろう(『x』):\n    『x』を返そう\n〈数字ではない〉を作ろう(『x』):\n    「はい」を返そう\n〈壊れた〉を作ろう(『x』):\n    新しい【エラー】(「コールバックが壊れました」)を発生させよう\n〈長さ〉を作ろう(『文』):\n    『文』の『長さ』を返そう\n〈何もしない〉を作ろう(『x』):\n    『y』を【数字】の1にしよう\n〈探す〉(【1, 2】, 〈数字ではない〉)を出力しよう\n" },
    { name: "표준: 목록 콜백 오류 3", code: "【リスト】から全部持ってこよう\n〈二倍〉を作ろう(『x』):\n    (『x』 * 2)を返そう\n〈偶数〉を作ろう(『x』):\n    もし(『x』 % 2 == 0)なら:\n        真を返そう\n    偽を返そう\n〈足す〉を作ろう(『合計』, 『x』):\n    (『合計』 + 『x』)を返そう\n〈自分〉を作ろう(『x』):\n    『x』を返そう\n〈数字ではない〉を作ろう(『x』):\n    「はい」を返そう\n〈壊れた〉を作ろう(『x』):\n    新しい【エラー】(「コールバックが壊れました」)を発生させよう\n〈長さ〉を作ろう(『文』):\n    『文』の『長さ』を返そう\n〈何もしない〉を作ろう(『x』):\n    『y』を【数字】の1にしよう\n〈変換〉(【1, 2】, 5)を出力しよう\n" },
    { name: "표준: 목록 콜백 오류 4", code: "【リスト】から全部持ってこよう\n〈二倍〉を作ろう(『x』):\n    (『x』 * 2)を返そう\n〈偶数〉を作ろう(『x』):\n    もし(『x』 % 2 == 0)なら:\n        真を返そう\n    偽を返そう\n〈足す〉を作ろう(『合計』, 『x』):\n    (『合計』 + 『x』)を返そう\n〈自分〉を作ろう(『x』):\n    『x』を返そう\n〈数字ではない〉を作ろう(『x』):\n    「はい」を返そう\n〈壊れた〉を作ろう(『x』):\n    新しい【エラー】(「コールバックが壊れました」)を発生させよう\n〈長さ〉を作ろう(『文』):\n    『文』の『長さ』を返そう\n〈何もしない〉を作ろう(『x』):\n    『y』を【数字】の1にしよう\n〈変換〉(【1, 2】, 〈壊れた〉)を出力しよう\n" },
    { name: "표준: 목록 콜백 오류 5", code: "【リスト】から全部持ってこよう\n〈二倍〉を作ろう(『x』):\n    (『x』 * 2)を返そう\n〈偶数〉を作ろう(『x』):\n    もし(『x』 % 2 == 0)なら:\n        真を返そう\n    偽を返そう\n〈足す〉を作ろう(『合計』, 『x』):\n    (『合計』 + 『x』)を返そう\n〈自分〉を作ろう(『x』):\n    『x』を返そう\n〈数字ではない〉を作ろう(『x』):\n    「はい」を返そう\n〈壊れた〉を作ろう(『x』):\n    新しい【エラー】(「コールバックが壊れました」)を発生させよう\n〈長さ〉を作ろう(『文』):\n    『文』の『長さ』を返そう\n〈何もしない〉を作ろう(『x』):\n    『y』を【数字】の1にしよう\n〈変換〉(「文」, 〈二倍〉)を出力しよう\n" },
    { name: "표준: 목록 콜백 오류 6", code: "【リスト】から全部持ってこよう\n〈二倍〉を作ろう(『x』):\n    (『x』 * 2)を返そう\n〈偶数〉を作ろう(『x』):\n    もし(『x』 % 2 == 0)なら:\n        真を返そう\n    偽を返そう\n〈足す〉を作ろう(『合計』, 『x』):\n    (『合計』 + 『x』)を返そう\n〈自分〉を作ろう(『x』):\n    『x』を返そう\n〈数字ではない〉を作ろう(『x』):\n    「はい」を返そう\n〈壊れた〉を作ろう(『x』):\n    新しい【エラー】(「コールバックが壊れました」)を発生させよう\n〈長さ〉を作ろう(『文』):\n    『文』の『長さ』を返そう\n〈何もしない〉を作ろう(『x』):\n    『y』を【数字】の1にしよう\n〈変換〉(【1, 2】)を出力しよう\n" },
    { name: "표준: 목록 콜백 오류 7", code: "【リスト】から全部持ってこよう\n〈二倍〉を作ろう(『x』):\n    (『x』 * 2)を返そう\n〈偶数〉を作ろう(『x』):\n    もし(『x』 % 2 == 0)なら:\n        真を返そう\n    偽を返そう\n〈足す〉を作ろう(『合計』, 『x』):\n    (『合計』 + 『x』)を返そう\n〈自分〉を作ろう(『x』):\n    『x』を返そう\n〈数字ではない〉を作ろう(『x』):\n    「はい」を返そう\n〈壊れた〉を作ろう(『x』):\n    新しい【エラー】(「コールバックが壊れました」)を発生させよう\n〈長さ〉を作ろう(『文』):\n    『文』の『長さ』を返そう\n〈何もしない〉を作ろう(『x』):\n    『y』を【数字】の1にしよう\n〈基準並べ替え〉(【1, 「a」】, 〈自分〉)を出力しよう\n" },
    { name: "표준: 목록 콜백 오류 8", code: "【リスト】から全部持ってこよう\n〈二倍〉を作ろう(『x』):\n    (『x』 * 2)を返そう\n〈偶数〉を作ろう(『x』):\n    もし(『x』 % 2 == 0)なら:\n        真を返そう\n    偽を返そう\n〈足す〉を作ろう(『合計』, 『x』):\n    (『合計』 + 『x』)を返そう\n〈自分〉を作ろう(『x』):\n    『x』を返そう\n〈数字ではない〉を作ろう(『x』):\n    「はい」を返そう\n〈壊れた〉を作ろう(『x』):\n    新しい【エラー】(「コールバックが壊れました」)を発生させよう\n〈長さ〉を作ろう(『文』):\n    『文』の『長さ』を返そう\n〈何もしない〉を作ろう(『x』):\n    『y』を【数字】の1にしよう\n〈基準並べ替え〉(【【1】, 【2】】, 〈自分〉)を出力しよう\n" },
    { name: "조건: 괄호로 시작하는 비교", code: "『x』を【数字】の4にしよう\nもし((『x』 % 2) == 0)なら:\n    「あ」を出力しよう\nもし(『x』 % 2 == 0)なら:\n    「い」を出力しよう\nもし(((『x』 * 2) + 1) >= 9)なら:\n    「う」を出力しよう\nもし(((『x』 % 2) == 1))なら:\n    「出ません」を出力しよう\n" },
    { name: "조건: 참거짓이 아닌 값 1", code: "『x』を【数字】の4にしよう\nもし(『x』)なら:\n    「出ません」を出力しよう\n" },
    { name: "조건: 참거짓이 아닌 값 2", code: "『x』を【文字列】の「文」にしよう\nもし(1 < 2)かつ(『x』)なら:\n    「出ません」を出力しよう\n" },
    { name: "조건: 참거짓이 아닌 값 3", code: "『x』を【数字】の1にしよう\n(『x』)間繰り返そう:\n    『x』を0にしよう\n" },
    { name: "조건: 참거짓이 아닌 값 4", code: "『x』を【数字】の1にしよう\nもし(1 < 2)または(『x』)なら:\n    「出ます」を出力しよう\n" },
    { name: "연산: 소수로 나눈 나머지는 0으로 나누기", code: "(7 % 0.5)を出力しよう\n" },
    { name: "분기: 맞는 경우가 없으면 아무 일도 없음", code: "『x』を【文字列】の「水」にしよう\n『x』によって分けよう:\n    「月」の場合:\n        「か」を出力しよう\n" },
    { name: "분기: 나머지는이 있으면 오류가 없음", code: "『x』を【文字列】の「水」にしよう\n『x』によって分けよう:\n    「月」の場合:\n        「か」を出力しよう\n    残りは:\n        「に」を出力しよう\n" },
    { name: "반복: 문자열 순회", code: "『あいさつ』を【文字列】の「こんにちは😀」にしよう\n『あいさつ』の『文字』ごとに繰り返そう:\n    『文字』を出力しよう\n" },
    { name: "반복: 반복 변수와 끝값을 바꿔도 횟수는 그대로", code: "『終わり』を【数字】の3にしよう\n1から『終わり』まで繰り返そう(『回』):\n    『回』を出力しよう\n    『終わり』に10を足そう\n    『回』に5を足そう\n" },
    // 반복 변수와 처리기의 오류 변수는 자기 범위에 새로 선언되어 같은 이름의 바깥 변수를 가린다 (스펙 1.1)
    { name: "범위: 반복 변수가 같은 이름의 바깥 변수를 가림", code: `『か』を100にしよう
1から2まで繰り返そう(『か』):
    『か』を出力しよう
『か』を出力しよう
` },
    { name: "범위: 범위의 끝은 가려지기 전의 바깥 변수", code: `『か』を3にしよう
1から『か』まで繰り返そう(『か』):
    『か』を出力しよう
『か』を出力しよう
` },
    { name: "범위: 가려진 바깥 상수와 같은 이름의 반복 변수는 바꿀 수 있음", code: `『か』を「字」に固定しよう
1から2まで繰り返そう(『か』):
    『か』に10を足そう
    『か』を出力しよう
『か』を出力しよう
` },
    { name: "범위: 가려진 상수 목록과 같은 이름의 항목에 추가할 수 있음", code: `『目録』を【1】に固定しよう
『中』を【5】にしよう
『外』を【『中』】にしよう
『外』の『目録』ごとに繰り返そう:
    『目録』の後ろに6を追加しよう
    『目録』を出力しよう
『目録』を出力しよう
` },
    { name: "범위: 마다의 항목이 바깥 변수를 가림", code: `『果物』を「元」にしよう
『果物たち』を【「りんご」,「なし」】にしよう
『果物たち』の『果物』ごとに繰り返そう:
    『果物』を出力しよう
『果物』を出力しよう
` },
    { name: "범위: 처리기의 오류 변수가 바깥 변수를 가림", code: `『エラー』を「元」にしよう
とりあえずやってみよう:
    新しい【エラー】(「失敗」)を発生させよう
発生したら(『エラー』):
    『エラー』の『メッセージ』を出力しよう
『エラー』を出力しよう
` },
    { name: "범위: 오류로 빠져나온 반복의 변수는 사라짐", code: `『か』を100にしよう
とりあえずやってみよう:
    1から3まで繰り返そう(『か』):
        『一時』を5にしよう
        新しい【エラー】(「止まる」)を発生させよう
発生したら(『e』):
    『e』の『メッセージ』を出力しよう
『か』を出力しよう
『一時』を出力しよう
` },
    // 반복 밖에서 실행된 繰り返しを終わろう는 IllegalBreakError (스펙 4.4)
    { name: "끝내기: 반복 밖에서 실행되면 IllegalBreakError", code: `「あ」を出力しよう
繰り返しを終わろう
「い」を出力しよう
` },
    { name: "끝내기: 반복 밖이어도 실행되지 않으면 괜찮음", code: `もし(1が2と同じだ)ならば:
    繰り返しを終わろう
「終わり」を出力しよう
` },
    { name: "끝내기: 함수 안의 반복을 끝내자는 부른 쪽의 반복을 끝내지 않음", code: `〈切る〉を作ろう():
    「f」を出力しよう
    繰り返しを終わろう
1から3まで繰り返そう(『数』):
    『数』を出力しよう
    とりあえずやってみよう:
        〈切る〉()を実行しよう
    発生したら(『e』):
        『e』の『メッセージ』を出力しよう
「終わり」を出力しよう
` },
    { name: "끝내기: 일단 해보자 안의 반복", code: `とりあえずやってみよう:
    1から5まで繰り返そう(『数』):
        もし(『数』が2と同じだ)ならば:
            繰り返しを終わろう
        『数』を出力しよう
    「後」を出力しよう
発生したら(『e』):
    「捕まえた」を出力しよう
最後はいつも:
    「F」を出力しよう
「終わり」を出力しよう
` },
    { name: "끝내기: 처리기에서 반복을 끝내면 마무리는 한 번", code: `1から3まで繰り返そう(『数』):
    とりあえずやってみよう:
        新しい【エラー】(「あ」)を発生させよう
    発生したら(『e』):
        繰り返しを終わろう
    最後はいつも:
        「F」を出力しよう
「終わり」を出力しよう
` },
    { name: "오류: 처리기 안의 오류도 마무리를 실행", code: `とりあえずやってみよう:
    とりあえずやってみよう:
        新しい【エラー】(「あ」)を発生させよう
    発生したら(『e』):
        新しい【エラー】(「い」)を発生させよう
    最後はいつも:
        「F内」を出力しよう
発生したら(『e』):
    『e』の『メッセージ』を出力しよう
最後はいつも:
    「F外」を出力しよう
` },
    { name: "범위: 끝이 숫자가 아니면 RangeMustBeNumbers", code: `『n』を「五」にしよう
1から『n』まで繰り返そう(『数』):
    『数』を出力しよう
` },
    { name: `반복: 목록 참조: 함수가 받은 목록에 추가하고 바꾸면 원본이 바뀜`, code: `〈埋める〉を作ろう(【(数字)リスト】の『中身』):
    『中身』の後ろに99を追加しよう
    『中身』の1番目を7にしよう

『あ』を【(数字)リスト】の【1,2】にしよう
〈埋める〉(『あ』)を実行しよう
『あ』を出力しよう
` },
    { name: `반복: 목록 참조: 함수가 받은 목록에서 꺼내면 원본이 바뀜`, code: `〈取る〉を作ろう(【(数字)リスト】の『中身』):
    『中身』の後ろから取り出そう

『あ』を【(数字)リスト】の【1,2,3】にしよう
〈取る〉(『あ』)を実行しよう
『あ』を出力しよう
` },
    { name: `반복: 목록 참조: 함수가 받은 목록을 비우면 원본이 비워짐`, code: `〈消す〉を作ろう(【(数字)リスト】の『中身』):
    『中身』の〈空にする〉()を実行しよう

『あ』を【(数字)リスト】の【1,2】にしよう
〈消す〉(『あ』)を実行しよう
『あ』を出力しよう
` },
    { name: `반복: 목록 참조: 같은 목록을 받은 두 변수는 하나를 공유함`, code: `『あ』を【(数字)リスト】の【1】にしよう
『い』を【(数字)リスト】の『あ』にしよう
『い』の後ろに2を追加しよう
『あ』を出力しよう
` },
    { name: `반복: 목록 참조: 반복은 시작할 때의 목록을 돎`, code: `『数たち』を【(数字)リスト】の【1,2,3】にしよう
『数たち』の『数』ごとに繰り返そう:
    『数たち』の後ろに『数』を追加しよう
『数たち』を出力しよう
` },
    { name: `반복: 목록 참조: 객체의 목록 필드는 객체를 통해 공유됨`, code: `【箱】を設計しよう:
    『中』を【(数字)リスト】の【1】にしよう

〈入れる〉を作ろう(【箱】の『入れ物』):
    『入れ物』の『中』の後ろに2を追加しよう

『はこ』を【箱】の新しい【箱】()にしよう
『別のはこ』を【箱】の新しい【箱】()にしよう
〈入れる〉(『はこ』)を実行しよう
『はこ』の『中』を出力しよう
『別のはこ』の『中』を出力しよう
` },
    { name: `반복: 목록 참조: 상수 목록에는 추가할 수 없음`, code: `『リスト』を【(数字)リスト】の【1,2】で固定しよう
『リスト』の後ろに3を追加しよう
` },
    { name: "반복: 숫자는 순회할 수 없음", code: "『n』を【数字】の3にしよう\n『n』の『文字』ごとに繰り返そう:\n    『文字』を出力しよう\n" },
];

// 구문 오류 문구: TS 엔진의 진단을 현지화한 문장이 hana가 보여 주는 문장과 같아야 한다.
const SYNTAX_CASES: string[] = ["「あ」を出力しよう )", "「あ」を出力しよう ＠", "1 +"];

function goSyntaxMessages(code: string): string[] {
    const tempFile = 'temp_syntax.knd';
    writeFileSync(tempFile, code);
    let stdout = '';
    try {
        stdout = execSync(`${GO_EXECUTABLE} run ${tempFile}`, { stdio: ['pipe', 'pipe', 'pipe'] }).toString();
    } catch (e: any) {
        stdout = e.stdout ? e.stdout.toString() : '';
    }
    unlinkSync(tempFile);
    return stdout.split(/\r?\n/).filter((l) => l.startsWith('  - ')).map((l) => l.slice(4));
}

function tsSyntaxMessages(code: string): string[] {
    const parser = new Parser(new Lexer(code).tokens);
    parser.parseProgram();
    return parser.diagnostics.map((d) => localize(JapaneseConfig.locale, syntaxError(d)));
}

async function main() {
    console.log("🔍 TypeScript 엔진 vs Go 엔진 출력 비교를 시작합니다 (Kanade)...\n");

    let total = 0;
    let passed = 0;

    await walk(DOCS_DIR, async (path) => {
        const content = readFileSync(path, 'utf8');
        const blocks = extractKanadeBlocks(content);

        for (let i = 0; i < blocks.length; i++) {
            total++;
            const code = blocks[i];

            const tsOutput = await runTypeScriptEngine(code);
            const goOutput = runGoEngine(code);

            if (NATIVE_ONLY.test(code) ? tsOutput.includes('ブラウザでは使えません') : (tsOutput === goOutput || (NONDETERMINISTIC.test(code) && !tsOutput.includes('!! ') && !goOutput.includes('오류') && !goOutput.includes('エラー')))) {
                passed++;
            } else {
                console.log(`❌ [불일치] 파일: ${path} (블록 ${i + 1})`);
                console.log(`-- 코드 --\n${code.trim()}`);
                console.log(`-- TS 출력 --\n${tsOutput}`);
                console.log(`-- Go 출력 --\n${goOutput}\n`);
            }
        }
    });

    // hana에는 있지만 브라우저에서는 쓸 수 없는 모듈(std의 NativeOnly): Go 출력과 비교하지 않고,
    // 브라우저 엔진이 전용 에러로 거절하는지만 본다.
    for (const c of BROWSER_ONLY_CASES) {
        total++;
        const tsOutput = await runTypeScriptEngine(c.code);
        if (tsOutput.includes(c.wantError)) {
            passed++;
        } else {
            console.log(`❌ [불일치] 브라우저 전용 케이스: ${c.name}`);
            console.log(`-- 기대한 에러 --
${c.wantError}
-- TS 출력 --
${tsOutput}
`);
        }
    }

    for (const c of EXTRA_CASES) {
        total++;
        const tsOutput = await runTypeScriptEngine(c.code, c.stdin);
        const goOutput = runGoEngine(c.code, c.stdin);
        if (tsOutput === goOutput) {
            passed++;
        } else {
            console.log(`❌ [불일치] 추가 케이스: ${c.name}`);
            console.log(`-- 코드 --
${c.code}`);
            console.log(`-- TS 출력 --
${tsOutput}`);
            console.log(`-- Go 출력 --
${goOutput}
`);
        }
    }

    for (const code of SYNTAX_CASES) {
        total++;
        const ts = tsSyntaxMessages(code).join('\n');
        const go = goSyntaxMessages(code).join('\n');
        if (ts === go && ts !== '') {
            passed++;
        } else {
            console.log(`❌ [불일치] 구문 오류 케이스: ${code}`);
            console.log(`-- TS --\n${ts}`);
            console.log(`-- Go --\n${go}\n`);
        }
    }

    console.log(`=========================================`);
    console.log(`총 실행된 코드 블록: ${total}개`);
    if (passed === total) {
        console.log(`✅ 모든 코드 블록의 출력이 완벽하게 일치합니다! (${passed}/${total})`);
    } else {
        console.log(`🚨 ${total - passed}개의 테스트 출력이 불일치합니다. (${passed}/${total})`);
        process.exit(1);
    }
}

main().catch(console.error);
