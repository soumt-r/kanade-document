...
カナデのASTは、ハジャ（Haja）とまったく同じノード構造です。パーサーが日本語のキーワードを読み取ったあとに生成されるJSONは、ハジャと完全に互換性があります。違いは区切り記号と語彙だけなので、各ノードの下に書いてあるカナデの書き方（`『変数』`、`【タイプ】`、`〈関数〉`）とJSONの対応を見れば、ハジャのドキュメントもそのまま読み替えられます。語彙の対応は[言語概要](/docs/tech-spec/1-overview)の表にまとめてあります。
