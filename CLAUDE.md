...
`cd ../hana && go run ./cmd/errsgen ../hari-docs/src/utils/hari/errCatalog.ts ../kanade-docs/src/utils/kanade/errCatalog.ts`
...
`cd ../hana && go run ./cmd/stdgen -hari ../hari-docs/src/utils/hari/stdNames.ts -kanade ../kanade-docs/src/utils/kanade/stdNames.ts`
...
`src/utils/kanade/lspKeywords.ts` も `hana/lsp` のキーワード表から**生成されるファイル**です(`kanadeLSP.ts` の補完が使用)。直接編集せず、`cd ../hana && go run ./cmd/lspgen -hari ../hari-docs/src/utils/hari/lspKeywords.ts -kanade ../kanade-docs/src/utils/kanade/lspKeywords.ts` で再生成します(`-check` で最新かどうか確認)。
...
チュートリアルのコードブロックは ` ```kanade `（ブラウザで実行できない例は ` ```kanade fail `）だけを使ってください。` ```text ` のような別の言語名だとサイトがコードブロックとして装飾せず、暗い背景に文字が見えなくなります（13章で実際に起きました）。`fail` ブロックは doctest が「エラーになれば合格」として検査します。ネットワークや相手のプログラムが必要な例は ` ```kanade skip ` と書くと、doctest と `compare_tests.ts` は実行せず表示だけします（14・15章）。コマンド・フォルダ構成・JSONのように実行しないテキストは ` ```bash ` か ` ```json ` を使ってください（言語名のない ` ``` ` は実行ボタン付きのエディタになります）。
