...
チュートリアルのコードブロックは ` ```kanade `（ブラウザで実行できない例は ` ```kanade fail `）だけを使ってください。` ```text ` のような別の言語名だとサイトがコードブロックとして装飾せず、暗い背景に文字が見えなくなります（13章で実際に起きました）。`fail` ブロックは doctest が「エラーになれば合格」として検査します。ネットワークや相手のプログラムが必要な例は ` ```kanade skip ` と書くと、doctest と `compare_tests.ts` は実行せず表示だけします（14・15章）。
